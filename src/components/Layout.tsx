...
import { 
  ...
  Settings, Palette, Wrench, ChevronDown, Search
} from 'lucide-react';

export const Layout: any = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [expandedMenus, setExpandedMenus] = useState<Record<string, boolean>>({ Settings: true });
  ...
  const menuItems = [
    ...
    { 
      ...
      subItems: [
        ...
        { name: 'Advanced', path: '/settings/advanced', icon: <Wrench className="w-4 h-4" /> }
      ]
    },
  ];
    ...
    <div className="flex h-screen bg-base-100 text-base-content font-sans overflow-hidden">
      ...
      >
        <div className="h-16 flex items-center justify-center border-b border-base-200 px-4">
          <span className={`font-bold text-lg tracking-wider text-base-content transition-opacity duration-200 ${isSidebarOpen ? 'opacity-100' : 'hidden'}`}>
            ERP
          </span>
          <span className={`font-bold text-lg tracking-wider text-primary transition-opacity duration-200 ${!isSidebarOpen ? 'opacity-100' : 'hidden'}`}>
            ERP
          </span>
        </div>
              ...
              {item.subItems ? (
                <>
                  ...
                  {isSidebarOpen && expandedMenus[item.name] && (
                    <div className="ml-6 pl-2 border-l-2 border-primary/30 flex flex-col gap-1 mt-1 overflow-hidden animate-fade-in">
          ...
          <div className={`flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'}`}>
            ...
            {isSidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate text-base-content">John Doe</p>
                <p className="text-xs truncate text-base-content/60">Administrator</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-base-100/80 backdrop-blur-sm border-b border-base-200 flex items-center justify-between px-6 sticky top-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex text-sm text-base-content/60">
              <span className="hover:text-base-content cursor-pointer transition-colors" onClick={() => navigate('/')}>Home</span>
              <span className="mx-2">/</span>
              <span className="text-base-content font-medium capitalize">{location.pathname.split('/').filter(Boolean).pop() || 'Dashboard'}</span>
            </div>
          </div>
